...
import { isStaleGroup, markNotification, unreadCount } from '$lib/server/notifications'

// PATCH /api/notifications/[id]?group_id= { read: boolean } → marquer lue / non lue
export const PATCH: RequestHandler = async ({ locals, params, request, url }) => {
	...
	const notification = await markNotification(locals.user.id, groupId, id, body.read)
